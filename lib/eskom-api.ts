...
class EskomAPI {
  ...
  async getStatus(areaId: string): Promise<Status | null> {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/area?id=${areaId}`, {
        headers: {
          Token: this.apiKey,
        },
      });
  ...
  private transformAreaData(data: ApiAreaResponse): Status {
  ...
  private parseSchedule(
    scheduleData?: ApiAreaResponse["schedule"]
  ): Schedule[] {
    if (!scheduleData || !scheduleData.days) {
      return this.getMockAreaInfo("default").schedule;
    }

    return scheduleData.days.map((day: ApiDay) => ({
      day: day.date,
      stages: day.stages.map((stageTimes: string[], index: number) => ({
        stage: index + 1,
        times: stageTimes || [],
      })),
    }));
  ...
  public getMockAreas(searchText: string): AreaInfo[] {
    const allAreas = [
      {
        id: "eskde-4-sandton-sandton",
        name: "Sandton, Johannesburg",
        region: "Gauteng",
      },
      {
        id: "eskde-10-pretoriacentral",
        name: "Pretoria Central",
        region: "Gauteng",
      },
      {
        id: "eskde-11-cape-town-cbd",
        name: "Cape Town CBD",
        region: "Western Cape",
      },
      {
        id: "eskde-6-durban-central",
        name: "Durban Central",
        ...
        schedule: [
          {
            ...
            stages: [
              {
                stage: 1,
                times: ["00:00-02:30", "08:00-10:30", "16:00-18:30"],
              },
              {
                stage: 2,
                times: ["00:00-02:30", "08:00-12:30", "16:00-20:30"],
              },
              {
                stage: 3,
                times: ["00:00-04:30", "08:00-12:30", "16:00-20:30"],
              },
            ],
          },
        ],
        ...
        schedule: [
          {
            ...
            stages: [
              {
                stage: 1,
                times: ["06:00-08:30", "14:00-16:30", "22:00-00:30"],
              },
              {
                stage: 2,
                times: ["06:00-10:30", "14:00-18:30", "22:00-02:30"],
              },
              {
                stage: 3,
                times: ["06:00-12:30", "14:00-20:30", "22:00-04:30"],
              },
            ],
          },
        ],
